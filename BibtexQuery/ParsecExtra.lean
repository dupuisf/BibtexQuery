...
open Lean Parsec.String Parsec
...
@[inline]
def noneOf (bad : String) : Parser Char := Parsec.satisfy (fun z => ¬bad.contains z)

@[inline]
def noneOfStr (bad : String) : Parser String := Parsec.manyChars (noneOf bad)
